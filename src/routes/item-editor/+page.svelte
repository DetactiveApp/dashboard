<script lang="ts">
    ...
    const pushItem =  async () => {
        fetch("/api/items", {method: "POST", body: JSON.stringify($appStore.itemEditor.cItem)});
    }
    ...
    <div class="flex flex-col items-center">
        ...
        <div class="flex flex-col justify-evenly mt-10 gap-5 w-3/4">
            <div class="flex gap-5">
                ...
                <input type="number"  placeholder="Probability" title="Set the value for the spawn probability from 0.0 to 1.0" class="input input-bordered w-32" bind:this={pInput} on:input={() => {
                    if (+pInput.value > 1.0) {
